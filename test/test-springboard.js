...
contract("Springboard", accounts => {
   ...
   it("Upgrade wallet v1 to v2 should work", async () => {
      ...
      await walletV1.die();
      const runtimeWallet2 = WalletV2.deployedBytecode;
      let t2 = await springboard.execute(runtimeWallet2);
      assert.equal(t2.logs.length, 1, "There is 1 event log");
      assert.equal(t2.logs[0].event, "contract", "another event");
